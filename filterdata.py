...
# req_data = {'country':'US', 'city': 'Chicago', 'from': '2023-12-20', 'to':'2023-12-31', 'Ext_type': 'heat-map' }
# Read data

list_data = []
data = pd.read_csv(f'C:/xampp/htdocs/php/PythonFlask/static/Data/{req_data["city"]}_crime_2001_2023.csv')

data.date = pd.to_datetime(data['date'], format='%Y-%m-%d %H:%M:%S')

fr_date = pd.to_datetime(req_data['from'], format='%Y-%m-%d %H:%M:%S')
to_date = pd.to_datetime(req_data['to'], format='%Y-%m-%d %H:%M:%S')
